...
VALUES ('user', 'testuser@example.com', '$2a$12$8LegtLQWe717tIPvZeivjuqKnaAs5.bm0Q05.5GrAmcKzXw2NjoUO', 0);
...
-- You can add similar updates for subtasks and minitasks based on completion in your application logic
UPDATE users SET score = score + 250 WHERE id = 1 AND (
    SELECT COUNT(*) FROM tasks WHERE user_id = 1 AND is_done = 1) > 0;

-- Updating scores based on existing completed data
UPDATE users SET score = score + 25 WHERE id = 1 AND (
    SELECT COUNT(*) FROM subtasks WHERE task_id = 1 AND is_done = 1) > 0;

UPDATE users SET score = score + 5 WHERE id = 1 AND (
